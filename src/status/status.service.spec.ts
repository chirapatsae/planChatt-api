...
jest.mock('../util/handleException', () => ({
  handleException: jest.fn((logger, error) => {
    throw error;
  }),
}));
...
describe('StatusService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(User), useFactory: mockUserRepository },
        {
          ...
          useFactory: mockWorkHistoryRepository,
        },
      ],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('should create and return a status (success)', async () => {
      ...
      statusRepository.create.mockReturnValue({
        ...sampleStatus,
        createdBy: sampleWorkHistory,
      });
    ...
    it('should handle edge case: empty name', async () => {
      ...
      statusRepository.save.mockResolvedValueOnce({
        ...sampleStatus,
        name: '',
      } as Status);
  ...
  describe('update', () => {
    ...
    it('should throw BadRequestException if name exists', async () => {
      statusRepository.findOne.mockResolvedValueOnce({
        ...sampleStatus,
        id: 'other-id',
      } as Status);
      await expect(service.update('status-uuid', updateDto)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('softRemove', () => {
    ...
    it('should soft remove a status', async () => {
      ...
      expect(result).toEqual({
        message: `Status ${sampleStatus.name} soft removed successfully`,
      });
    });
    it('should throw BadRequestException if status not found', async () => {
      statusRepository.findOne.mockResolvedValueOnce(null);
      await expect(service.softRemove('not-exist', userId)).rejects.toThrow(
        BadRequestException,
      );
    });
    it('should throw UnauthorizedException if work history not found', async () => {
      statusRepository.findOne.mockResolvedValueOnce(sampleStatus as Status);
      workHistoryRepository.findOne.mockResolvedValueOnce(null);
      await expect(service.softRemove('status-uuid', userId)).rejects.toThrow(
        UnauthorizedException,
      );
    });
    it('should throw InternalServerErrorException on DB error', async () => {
    ...
    it('should handle edge case: empty id', async () => {
      statusRepository.findOne.mockResolvedValueOnce(null);
      await expect(service.softRemove('', userId)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('remove', () => {
    it('should hard delete a status', async () => {
      ...
      expect(result).toEqual({
        message: `Status with ID status-uuid has been permanently removed.`,
      });
    });
  ...
  describe('restore', () => {
    it('should restore a status', async () => {
      ...
      expect(result).toEqual({
        message: `Status with ID status-uuid has been restored`,
      });
    });
    ...
    it('should throw BadRequestException if restore.affected === 0', async () => {
      ...
      await expect(service.restore('status-uuid')).rejects.toThrow(
        BadRequestException,
      );
    });
    it('should throw InternalServerErrorException on DB error', async () => {
