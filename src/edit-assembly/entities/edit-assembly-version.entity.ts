...
/**
 * EditAssemblyVersion — Wave A2 / DB-01.
 *
 * Immutable published-version row produced by `finalize` on an
 * edit-revision book. Dedicated table; NOT shared with
 * `book_assembly_versions` / `main_assembly_versions`. Q3=B duplicate of
 * the BookAssembly shape — same columns minus the `(source_type,
 * source_id)` discriminator pair (replaced by a single
 * `development_plan_revision_id` FK column).
 *
 * Key shape differences vs `book_assembly_versions`:
 *   - `source_type` removed (always edit_revision; implicit by table)
 ...
 *     `edit_assembly_version_projects` join table per Wave A2 spec.
 ...
 *     `idx_edit_version_revision_number`).
 *   - Partial unique `idx_edit_single_completed_per_revision` enforces
 ...
 */
@Entity('edit_assembly_versions')
@Index('idx_edit_version_revision', ['developmentPlanRevisionId'])
@Index(
  'idx_edit_version_revision_number',
  ['developmentPlanRevisionId', 'versionNumber'],
  { unique: true },
)
@Index('idx_edit_version_status', ['status'])
export class EditAssemblyVersion {
  ...
  @Column({
    name: 'status',
    type: 'enum',
    enum: EditAssemblyVersionStatus,
    enumName: 'edit_assembly_version_status',
    default: EditAssemblyVersionStatus.COMPLETED,
  })
  ...
  @Column({
    name: 'part1_source',
    type: 'enum',
    enum: EditAssemblyPartSource,
    enumName: 'edit_assembly_part_source',
  })
