/**
 * ProjectAlignmentSdg — Junction row carrying SECONDARY SDG entries
 * for a single `project_alignment_mapping` triple.
 *
 * SOURCE OF TRUTH for ALL new read paths that need the full list of
 * SDG values attached to a given alignment triple. The legacy scalar
 * FK `project_alignment_mapping.sdg_id` remains on the parent row
 * purely for backward compatibility per the Scalar-FK Deprecation
 * Contract — see README §Scalar-FK Deprecation Contract in
...
@Entity({ name: 'project_alignment_sdg' })
@Unique('UQ_pa_sdg_pair', ['mappingId', 'sdgId'])
@Index('IDX_pa_sdg_mapping', ['mappingId'])
...
@Index('IDX_pa_sdg_sdg', ['sdgId'])
export class ProjectAlignmentSdg {
  ...
  @ManyToOne(() => ProjectAlignmentMapping, (m) => m.sdgs, {
    onDelete: 'CASCADE',
  })
  ...
  /**
   * Display order. The scalar `mapping.sdg_id` is treated as
   * `sort_order = 0` (the back-compat primary mirror). Junction rows
