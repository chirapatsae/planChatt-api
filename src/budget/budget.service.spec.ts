...
describe('BudgetService', () => {
  ...
  describe('create', () => {
    ...
    it('should create and return a budget (success)', async () => {
      ...
      expect(projectGroupRepo.findOne).toHaveBeenCalledWith({ where: { id: dto.projectGroupId }, relations: ['budgetPlanId'] });
      expect(budgetRepo.create).toHaveBeenCalledWith({ ...dto, projectGroupId: { id: dto.projectGroupId } });
    ...
    it('should throw BadRequestException if year is out of plan range', async () => {
      projectGroupRepo.findOne.mockResolvedValue(projectGroup as any);
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new BadRequestException(); });
      await expect(service.create({ ...dto, year: 2019 })).rejects.toThrow(BadRequestException);
      await expect(service.create({ ...dto, year: 2026 })).rejects.toThrow(BadRequestException);
    });
    ...
    it('should handle edge cases (empty/invalid input)', async () => {
      projectGroupRepo.findOne.mockResolvedValue(null as any);
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new NotFoundException(); });
      await expect(service.create({ ...dto, projectGroupId: '' })).rejects.toThrow(NotFoundException);
      await expect(service.create({ ...dto, year: 0 })).rejects.toThrow(NotFoundException);
      await expect(service.create({ ...dto, year: -1 })).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findAll', () => {
    ...
    it('should return all budgets (success)', async () => {
      ...
      expect(budgetRepo.find).toHaveBeenCalledWith({ where: {}, relations: ['projectGroupId', 'projectVersionId', 'budgetPlanId'] });
    });
    ...
    it('should filter by groupId', async () => {
      ...
      expect(result).toEqual(budgets);
      expect(budgetRepo.find).toHaveBeenCalledWith({ where: { projectGroupId: { id: 'group-uuid' } }, relations: ['projectGroupId', 'projectVersionId', 'budgetPlanId'] });
    });

    it('should throw InternalServerErrorException on DB error', async () => {
      budgetRepo.find.mockImplementation(async () => { throw new Error('DB error'); });
  ...
  describe('findOne', () => {
    it('should return a budget by id (success)', async () => {
      ...
      expect(budgetRepo.findOne).toHaveBeenCalledWith({ where: { id: 'budget-uuid' }, relations: ['projectGroupId', 'projectVersionId', 'budgetPlanId'] });
    });
    ...
    it('should throw InternalServerErrorException on DB error', async () => {
      ...
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new InternalServerErrorException(); });
      await expect(service.findOne('budget-uuid')).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe('update', () => {
    ...
    it('should update and return the budget (success)', async () => {
      ...
      expect(budgetRepo.preload).toHaveBeenCalledWith({ id: 'budget-uuid', quantity: updateDto.quantity });
    ...
    it('should throw NotFoundException if budget not found', async () => {
      ...
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new NotFoundException(); });
      await expect(service.update('not-exist', updateDto)).rejects.toThrow(NotFoundException);
    });

    it('should throw InternalServerErrorException on DB error', async () => {
      budgetRepo.preload.mockRejectedValue(new Error('DB error'));
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new InternalServerErrorException(); });
      await expect(service.update('budget-uuid', updateDto)).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe('remove', () => {
    it('should remove a budget (success)', async () => {
      ...
      expect(result).toEqual({ message: 'Budget with ID budget-uuid has been removed successfully.' });
    ...
    it('should throw InternalServerErrorException on DB error', async () => {
      ...
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new InternalServerErrorException(); });
      await expect(service.remove('budget-uuid')).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe('softRemove', () => {
    it('should soft remove a budget (success)', async () => {
      ...
      expect(result).toEqual({ message: 'budet with ID budget-uuid has been soft-removed.' });
    ...
    it('should throw NotFoundException if not found', async () => {
      ...
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new NotFoundException(); });
      await expect(service.softRemove('not-exist')).rejects.toThrow(NotFoundException);
    });

    it('should throw InternalServerErrorException on DB error', async () => {
      budgetRepo.softDelete.mockRejectedValue(new Error('DB error'));
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new InternalServerErrorException(); });
      await expect(service.softRemove('budget-uuid')).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe('restore', () => {
    it('should restore a budget (success)', async () => {
      ...
      expect(result).toEqual({ message: 'budet with ID budget-uuid has been restored.' });
    ...
    it('should throw NotFoundException if not found or not deleted', async () => {
      ...
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new NotFoundException(); });
      await expect(service.restore('not-exist')).rejects.toThrow(NotFoundException);
    });

    it('should throw InternalServerErrorException on DB error', async () => {
      budgetRepo.restore.mockRejectedValue(new Error('DB error'));
      handleExceptionSpy = jest.spyOn(handleExceptionModule, 'handleException').mockImplementation(() => { throw new InternalServerErrorException(); });
      await expect(service.restore('budget-uuid')).rejects.toThrow(InternalServerErrorException);
    });
