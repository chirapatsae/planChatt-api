...
export class GovernmentAgenciesService {
  ...
  async findOne(id: string) {
    try {
      ...
      if (!governmentAgency) {
        throw new NotFoundException(`Government Agency with ID ${id} not found`)
      }
  ...
  async update(id: string, updateGovernmentAgencyDto: UpdateGovernmentAgencyDto) {
    try {
      const governmentAgencyToUpdate = await this.governmentAgencyRepository.preload({
        id,
      ...
      return await this.governmentAgencyRepository.save(governmentAgencyToUpdate);
    } catch (error) {
      handleException(this.logger, error);
    }
  }
  ...
  async remove(id: string): Promise<{ message: string }> {
    try {
      ...
      return { message: `Government Agency with ID ${id} has been permanently removed.` };
    } catch (error) {
      handleException(this.logger, error);
    }
  }
  ...
  async softRemove(id: string): Promise<{ message: string }> {
    try {
      ...
      return { message: `Government Agency with ID ${id} has been soft-removed.` };
    } catch (error) {
      handleException(this.logger, error);
    }
  }
  ...
  async restore(id: string): Promise<{ message: string }> {
    try {
      ...
      if (result.affected === 0) {
        throw new NotFoundException(`Government Agency with ID ${id} not found or was not deleted.`);
      }
