...
import {
  MUNICIPAL_PROFILE,
  municipalAreaLabel,
  buildMunicipalContextBlock,
} from './municipal-profile';
import {
...
export class AiService {
  ...
  async generateProjectDetail(dto: GenerateProjectDto, userId: string) {
    ...
    // Wave 34 N1 — resolve budget floor ONCE per request. Single-อปท
    // rescope: every project belongs to เทศบาลตำบลหนองกระทุ่ม, so when the
    // caller omits organizationType (all internal กอง/สำนัก users are
    // agency-classified and carry no LAO type) we fall back to the municipal
    // profile's type. This keeps the budget clause + floor active for every
    // request instead of silently dropping the budget card for agency users.
    const budgetFloor = resolveBudgetFloor(
      dto.organizationType ?? MUNICIPAL_PROFILE.laoType,
    );
    ...
    // Municipal analyst persona (single-อปท). Identity comes from
    // MUNICIPAL_PROFILE (hardcoded fallback) — NOT from the DTO/DB — so the
    // model never defaults to อบจ./provincial voice. The no-fabrication rule
    // (ข้อ 5) replaces the old "cite ≥2 area statistics" directive until a
    // factual dossier exists.
    const baseSystemPrompt = `คุณคือนักวิเคราะห์นโยบายและแผนของ${MUNICIPAL_PROFILE.laoName} อำเภอ${MUNICIPAL_PROFILE.amphoeName} จังหวัด${MUNICIPAL_PROFILE.changwatName}
มีหน้าที่ร่างโครงการเพื่อบรรจุในแผนพัฒนาท้องถิ่นของเทศบาล ตามระเบียบกระทรวงมหาดไทยว่าด้วยการจัดทำแผนพัฒนาขององค์กรปกครองส่วนท้องถิ่น พ.ศ. 2548 และที่แก้ไขเพิ่มเติม (ฉบับที่ 3) พ.ศ. 2561

หลักการทำงาน:
1. โครงการต้องดำเนินการภายในเขต${MUNICIPAL_PROFILE.laoName} และอยู่ในอำนาจหน้าที่ของเทศบาลตำบลตามกฎหมาย
2. ต้องตอบสนองปัญหาและความต้องการจริงของประชาชนในพื้นที่${municipalAreaLabel()}
3. ต้องสอดคล้องอย่างชัดเจนกับยุทธศาสตร์ กลยุทธ์ และแผนงานที่ผู้ใช้เลือก โดยระบุความเชื่อมโยงในเนื้อหา
4. ห้ามระบุหรือสื่อว่าโครงการเป็นของ อบจ. องค์การบริหารส่วนจังหวัด จังหวัด หรือ อปท. อื่น
5. ห้ามกุตัวเลขสถิติ — อ้างอิงเฉพาะข้อมูลที่ให้ไว้ในบริบท หากไม่มีข้อมูลตัวเลข ให้เขียนเชิงพรรณนาโดยไม่ระบุตัวเลข
6. ใช้ภาษาราชการไทยที่ถูกต้องและสละสลวย ให้รายละเอียดครบถ้วนตามหัวข้อที่กำหนด`;
    ...
    // Inject the municipal grounding block UNCONDITIONALLY (both report
    // formats, with or without pinned coordinates) so every draft is anchored
    // to เทศบาลตำบลหนองกระทุ่ม even before the map step.
    const municipalContextBlock = buildMunicipalContextBlock();
    const systemPrompt = criteriaBlock
      ? `${baseSystemPrompt}\n\n${municipalContextBlock}\n\n${criteriaBlock}`
      : `${baseSystemPrompt}\n\n${municipalContextBlock}`;
...
ข้อกำหนดสำคัญ: ทุกหัวข้อต้องสะท้อนความสอดคล้องกับยุทธศาสตร์ "${dto.strategy || ''}" กลยุทธ์ "${dto.tactic || ''}" และแผนงาน "${dto.plan || ''}" ข้างต้น และตอบสนองปัญหา/ความต้องการของประชาชนในพื้นที่${municipalAreaLabel()} โดยแทรกความเชื่อมโยงไว้ในเนื้อหาของแต่ละหัวข้อ ตอบเฉพาะหัวข้อที่กำหนดไว้ด้านล่างเท่านั้น ห้ามสร้างหัวข้อใหม่นอกเหนือจากที่ระบุ
...
✓ ระบุตัวชี้วัดความสำเร็จที่วัดได้ พร้อมตัวเลขหรือร้อยละเป้าหมายที่ชัดเจน เขียนเป็นภาษาราชการไทยล้วน ห้ามใช้ศัพท์อังกฤษหรือรูปแบบสมการ เช่น "baseline = 0, target = 200"
...
**ตัวชี้วัด:**
[ตัวชี้วัดความสำเร็จที่วัดผลได้ 4–6 ประโยค เขียนเป็นภาษาราชการไทยล้วน ระบุตัวชี้วัดเชิงปริมาณ (เช่น จำนวนผู้เข้าร่วม จำนวนกิจกรรม พื้นที่ดำเนินการ) และเชิงคุณภาพ (เช่น ระดับความพึงพอใจ ความรู้ที่เพิ่มขึ้น) พร้อมตัวเลขหรือร้อยละเป้าหมายที่ชัดเจน อธิบายวิธีการวัดและแหล่งข้อมูล ครอบคลุมทั้งผลผลิตและผลลัพธ์ของโครงการ — **ห้าม**ใช้ศัพท์ภาษาอังกฤษหรือรูปแบบสมการ เช่น "baseline = 0, target = 200" / "output" / "outcome" ให้เขียนเป็นประโยคภาษาไทยแทน เช่น "มีเกษตรกรเข้าร่วมอบรมและใช้เทคโนโลยีไม่น้อยกว่า 200 ราย"]
...
1. "ข้อมูลที่มี": บริบทพื้นที่แบบละเอียด ให้ 3–5 ประโยค
   - อธิบายลักษณะสำคัญของพื้นที่${municipalAreaLabel()} (เศรษฐกิจ ชุมชน ภูมิประเทศ วัฒนธรรม)
   - อ้างอิงเฉพาะข้อเท็จจริง/ตัวเลขที่ปรากฏใน [MUNICIPAL_CONTEXT] เท่านั้น หากไม่มีข้อมูลตัวเลข ให้เขียนเชิงพรรณนาโดยไม่ระบุตัวเลข — ห้ามสร้างสถิติหรือปี พ.ศ. ขึ้นเอง
   - ใช้ภาษาราชการไทยที่สละสลวย

2. "เหตุผลที่คิดโครงการนี้": อธิบายเหตุผลเชิงลึก ให้ 3–5 ประโยค
   - เชื่อมโยงโครงการกับลักษณะพื้นที่และปัญหา/ความต้องการของประชาชนใน${municipalAreaLabel()}
   - อ้างอิงเฉพาะข้อเท็จจริงที่ให้ไว้ในบริบทเท่านั้น ห้ามกุตัวเลขสถิติ
...
รูปแบบการตอบ: ใช้หัวข้อภาษาไทยตามรูปแบบด้านบนทุกประการ (ชื่อโครงการ, วัตถุประสงค์, เป้าหมาย, ผลที่คาดว่าจะได้รับ, งบประมาณ, ตัวชี้วัด) — **ต้องมีหัวข้อ "งบประมาณ:" เสมอ** พร้อมเพิ่ม **ข้อมูลที่มี:**, **เหตุผลที่คิดโครงการนี้:**, **ความเหมาะสมของพื้นที่:**, และ **ป้ายพื้นที่:** ที่ท้ายสุด ตามลำดับนี้`;
...
ข้อกำหนดสำคัญ: ทุกหัวข้อต้องสะท้อนความสอดคล้องกับประเด็นการพัฒนา "${issueName}" และตอบสนองปัญหา/ความต้องการของประชาชนในพื้นที่${municipalAreaLabel()} โดยแทรกความเชื่อมโยงไว้ในเนื้อหาของแต่ละหัวข้อ ตอบเฉพาะหัวข้อที่กำหนดไว้ด้านล่างเท่านั้น ห้ามสร้างหัวข้อใหม่นอกเหนือจากที่ระบุ
...
1. "ข้อมูลที่มี": บริบท**ขอบเขตของ${MUNICIPAL_PROFILE.laoName}** (ไม่ใช่พิกัดที่ปักหมุด) ให้ 5–7 ประโยค (อย่างน้อย 120 คำ)
   - **สำคัญมาก — ขอบเขต**: ส่วนนี้ต้องอธิบาย ${municipalAreaLabel()} (จาก [MUNICIPAL_CONTEXT]) เท่านั้น ห้ามนำข้อมูลจากพิกัดที่ปักหมุด (เช่น ลักษณะแหล่งน้ำ ตำบลของพิกัด) มาใช้ในส่วนนี้ — ข้อมูลพิกัดใช้ในส่วน "ความเหมาะสมของพื้นที่" เท่านั้น
   - อธิบายลักษณะสำคัญของ**${municipalAreaLabel()}** (เศรษฐกิจ ชุมชน ภูมิประเทศ วัฒนธรรม ทรัพยากร ลักษณะประชากร บริการสาธารณะที่มีอยู่)
   - อ้างอิงเฉพาะข้อเท็จจริง/ตัวเลขที่ปรากฏใน [MUNICIPAL_CONTEXT] เท่านั้น หากไม่มีข้อมูลตัวเลข ให้เขียนเชิงพรรณนาโดยไม่ระบุตัวเลข — ห้ามสร้างสถิติหรือปี พ.ศ. ขึ้นเอง
...
2. "เหตุผลที่คิดโครงการนี้": อธิบายเหตุผลเชิงลึก ให้ 5–7 ประโยค
   ...
   - หากอ้างอิงตัวเลข ให้ใช้เฉพาะที่ปรากฏในบริบทเท่านั้น ห้ามกุตัวเลขขึ้นเอง
   - ระบุประโยชน์ที่คาดว่าจะเกิดและกลุ่มเป้าหมาย
...
[END_OUTPUT_HYGIENE]`;
    ...
    prompt += `\n\nรูปแบบการตอบ: ใช้หัวข้อภาษาไทยตามรูปแบบด้านบนทุกประการ (ชื่อโครงการ, วัตถุประสงค์, เป้าหมาย, ผลที่คาดว่าจะได้รับ, งบประมาณ) — **ต้องมีหัวข้อ "งบประมาณ:" เสมอ** พร้อมเพิ่ม **ข้อมูลที่มี:**, **เหตุผลที่คิดโครงการนี้:**, **ความเหมาะสมของพื้นที่:**, และ **ป้ายพื้นที่:** ที่ท้ายสุด ตามลำดับนี้
...
- หากไม่มีการอ้างอิงที่มั่นใจ ให้ละเว้นบล็อคนี้ทั้งหมด ห้ามกุข้อมูล (zero-fabrication) — ระบบจะไม่แสดงบล็อคว่าง`;
  ...
  async regenerateField(dto: RegenerateFieldDto, userId: string) {
    ...
    const lengthGuidanceMapping: { [key: string]: string } = {
      ...
      indicator:
        '- ให้ความยาวประมาณ 4–6 ประโยค สำหรับ ตัวชี้วัด เขียนเป็นภาษาราชการไทยล้วน ระบุตัวชี้วัดเชิงปริมาณและเชิงคุณภาพ พร้อมตัวเลขหรือร้อยละเป้าหมายที่ชัดเจน อธิบายวิธีการวัด ครอบคลุมทั้งผลผลิตและผลลัพธ์ — ห้ามใช้ศัพท์อังกฤษหรือรูปแบบสมการ เช่น "baseline = 0, target = 200" / "output" / "outcome" ให้เขียนเป็นประโยคภาษาไทยแทน',
    };
  ...
  private async executeLlmSmartApproveAnalysis(
    ...
    const systemPrompt =
      `คุณคือนักวิเคราะห์นโยบายและแผนของ${MUNICIPAL_PROFILE.laoName} มีหน้าที่ประเมินโครงการตามกรอบราชการไทยอย่างรอบคอบและเป็นมืออาชีพ กำหนดผลการประเมินในรูปแบบ JSON ตาม schema ที่กำหนดเท่านั้น`;
...
${instructions}`.trim();
  ...
  async generatePreSubmitReview(dto: PreSubmitReviewDto, userId: string) {
    ...
    const systemPrompt =
      `คุณคือนักวิเคราะห์นโยบายและแผนของ${MUNICIPAL_PROFILE.laoName} มีหน้าที่ตรวจประเมินคุณภาพร่างโครงการก่อนบรรจุในแผนพัฒนาท้องถิ่นของเทศบาล และให้คำแนะนำเชิงสร้างสรรค์แบบมืออาชีพ ประเมินในฐานะเจ้าหน้าที่ของเทศบาล ไม่ใช่ อบจ. หรือหน่วยงานระดับจังหวัด ใช้ภาษาราชการไทยที่สุภาพ กระชับ และตรงประเด็น ตอบเป็น JSON เท่านั้น` +
      (criteriaSystemTail ? `\n\n${criteriaSystemTail}` : '');
