...
export const createRevisionEditGroupCoverPageDocDefinitionUser = (
  strategyName: string,
  developmentPlanRevisionName: string,
  pageMargins: [number, number, number, number],
  pageOrientation: 'portrait' | 'landscape',
...
// สร้าง detail page สำหรับแต่ละ group (มี header "แบบ ผ.02" ทุกหน้า) - สำหรับ user version ที่แสดงยุทธศาสตร์ กลยุทธ์ แผนงานในโครงการใหม่
export const createRevisionEditGroupDetailDocDefinitionUser = (
  ...
  const createTableForOldProject = (
    ...
    if (showHeader) {
      // แถวหัวตาราง "ยุทธศาสตร์ กลยุทธ์ แผนงาน" - ใช้จาก previous
  ...
  // Helper function เพื่อสร้าง table สำหรับโครงการใหม่ (additionalDetail + current) - แสดงยุทธศาสตร์ กลยุทธ์ แผนงานด้วย
  const createTableForNewProject = (
    ...
    // ตรวจสอบว่ามีการเปลี่ยนแปลงหรือไม่ (ใช้ก่อนสร้างแถว)
    const currStrategyName = current.strategy?.name || strategyName || '-';
    const currTacticName = current.tactic?.name || tacticName || '-';
    const currPlanName = current.plan?.name || planName || '-';
    const strategyChanged = previous ? hasChanged(previous.strategy?.name, current.strategy?.name) : false;
    const tacticChanged = previous ? hasChanged(previous.tactic?.name, current.tactic?.name) : false;
    const planChanged = previous ? hasChanged(previous.plan?.name, current.plan?.name) : false;

    // แถวที่ 1: ยุทธศาสตร์ กลยุทธ์ แผนงาน (ของโครงการใหม่) - บนสุดก่อน header
    const strategyTacticPlanRow = [
      {
        colSpan: totalColumns,
        stack: [
          { 
            text: `ยุทธศาสตร์: ${currStrategyName}`, 
            bold: strategyChanged || !previous // ทำตัวหนาถ้าเปลี่ยนหรือไม่มี previous
          },
          { 
            text: `กลยุทธ์: ${currTacticName}`, 
            bold: tacticChanged || !previous, // ทำตัวหนาถ้าเปลี่ยนหรือไม่มี previous
            margin: [20, 0, 0, 0] 
          },
          { 
            text: `แผนงาน: ${currPlanName}`, 
            bold: planChanged || !previous, // ทำตัวหนาถ้าเปลี่ยนหรือไม่มี previous
            margin: [40, 0, 0, 0] 
          },
        ],
        border: [false, false, false, false],
    ...
    // แถวที่ 2: additionalDetail (colspan ทั้งแถว)
    const additionalDetailRow = additionalDetail ? [
      {
        colSpan: totalColumns,
    ...
    // แถวที่ 3: รายละเอียดโครงการใหม่ (current) - คอลัมน์ที่มีการเปลี่ยนแปลงให้ทำตัวหนา
    const newRow: any[] = [];
    groupColumns.forEach(col => {
    ...
    // สร้าง tableBody ตามลำดับที่ถูกต้อง (เหมือนโครงการเดิม):
    // 1. ยุทธศาสตร์ กลยุทธ์ แผนงาน (แถว 0) - ใช้ unshift เพื่อให้อยู่บนสุด
    // 2. Header rows (แถว 1, 2)
    // 3. additionalDetail (แถว 3, ถ้ามี)
    // 4. รายละเอียดโครงการ (แถว 4, newRow)
    const tableBody: any[] = [];
    
    // ใส่ยุทธศาสตร์ กลยุทธ์ แผนงานไว้แถวแรก (เหมือนโครงการเดิม)
    tableBody.push(strategyTacticPlanRow);
    
    // ใส่ header rows หลังยุทธศาสตร์ (ใช้ splice เหมือนโครงการเดิม)
    tableBody.splice(1, 0, headerRow1, headerRow2);
    
    // ใส่ additionalDetail (ถ้ามี)
    if (additionalDetailRow) {
      tableBody.push(additionalDetailRow);
    }
    
    // ใส่รายละเอียดโครงการ
    ...
    // ใช้ headerRows = 3 เหมือนโครงการเดิม (รวมยุทธศาสตร์ กลยุทธ์ แผนงาน + header 2 แถว)
    const headerRows = 3;
    ...
    return {
      ...
      layout: {
        hLineWidth: (i: number, node: any) => {
          // ไม่วาดเส้นด้านบนของแถวแรก (ยุทธศาสตร์ กลยุทธ์ แผนงาน) - เหมือนโครงการเดิม
          if (i === 0) return 0;
          // ไม่วาดเส้นด้านบนของแถวที่มี stack (ยุทธศาสตร์ กลยุทธ์ แผนงาน)
          if (node.table.body[i]?.[0]?.stack) return 0;
          return 0.3;
        },
        vLineWidth: () => 0.3,
