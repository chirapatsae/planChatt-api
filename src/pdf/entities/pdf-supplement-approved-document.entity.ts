...
/**
 * PdfSupplementApprovedDocument — SUPP_PRINT_DB_01.
 *
 * Persists generated supplement PDFs in the APPROVED variant (Q2 =
 * default flavor only). Mirrors the shape of
 * `PdfRevisionEditApprovedDocument` byte-for-byte; only the parent-book
 * FK differs.
 *
 ...
 * same repository pattern it already uses for revision approved docs.
 *
 * Out of scope (per task Q4=B):
 *   - Out-authority / Rejected document variant — deferred to
 *     SUPP_PRINT_WAVE_B.
 *
 ...
 *     hard-removed.
 */
@Entity('pdf_supplement_approved_documents')
export class PdfSupplementApprovedDocument {
